...
class Network_March2(torch.nn.Module):
    # Simple net
    def __init__(self):
        super(Network_March2, self).__init__()

        self.dropout_2d = torch.nn.Dropout2d(0.1)
        self.dropout_1d = torch.nn.Dropout(0.1)
        self.pool1 = torch.nn.MaxPool2d(kernel_size=2, stride=2, padding=0)

        self.conv1 = torch.nn.Conv2d(8, 4, kernel_size=5, stride=5, padding=0)
        self.conv1_bn = torch.nn.BatchNorm2d(4)

        self.conv2 = torch.nn.Conv2d(4, 2, kernel_size=3, stride=3, padding=0)
        self.conv2_bn = torch.nn.BatchNorm2d(2)
        ...
        self.fc1 = torch.nn.Linear(1 * 7 * 7, 32)
        self.fc1_bn = torch.nn.BatchNorm1d(32)

        self.fc_out = torch.nn.Linear(32, 3)

    def forward(self, x):
        x = F.relu(self.conv1_bn(self.conv1(x)))  # Size changes to 4,180,180
        x = self.pool1(x)  # Size changes to 4,90,90
        x = self.dropout_2d(x)

        x = F.relu(self.conv2_bn(self.conv2(x))) # Size changes to 2,30,30
        x = self.pool1(x)  # Size changes to 2,15,15
        x = self.dropout_2d(x)

        x = F.relu(self.conv3_bn(self.conv3(x))) # Size changes to 2,15,15
        x = self.pool1(x)  # Size changes to 2,7,7
        x = self.dropout_2d(x)

        x = F.relu(self.conv4_bn(self.conv4(x))) # Size changes to 1,7,7
        x = self.dropout_2d(x)
