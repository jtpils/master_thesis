...
def train_network(net, train_loader, val_loader, n_epochs, learning_rate, patience, folder_path, use_cuda):
    ...
    for epoch in range(n_epochs):
        ...
        running_loss = 0.0
        print_every = n_batches // np.ceil(n_batches/10)  # how many mini-batches if we want to print stats x times per epoch
        ...
        for i, data in enumerate(train_loader, 1):
            ...
            if use_cuda:
                ...
                sample, labels = Variable(sample).to("cuda:0"), Variable(labels).to("cuda:0")
            else:
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                if use_cuda:
                    ...
                    sample, labels = Variable(sample).to("cuda"), Variable(labels).to("cuda")
